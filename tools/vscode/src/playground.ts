...
// CONTRACT: this shape is a copy of the playground's own share model
// (samples/PqJwtPlayground, `ShareState`, serialized as System.Text.Json
// camelCase and read back from the `?c=` param). There is no shared schema, so
// the two must be kept in sync by hand — if the playground adds/renames a field,
// update this interface and buildShareState() or the deep-link silently degrades
// (the playground ignores unknown fields and defaults missing ones). The fields
// below are intentionally claims/options only — never key material.
